...
from flaskDemo.forms import RegistrationForm, LoginForm, UpdateAccountForm, PostForm, DeptForm
...
@app.route("/home")
def home():
    results= Department.query.with_entities(Department.mgr_ssn).all()
    return render_template('ssn_home.html', outString = results)
...
def new_dept():
    form = DeptForm()
    if form.validate_on_submit():
        print('**********************************form.mgr_ssn.data',form.mgr_ssn.data)
        print('**********************************form.dname.data',form.dname.data)
        result = Department.query.get(form.mgr_ssn.data)
        print (result.mgr_ssn)
        print(result['mgr_ssn'])
        dept = Department(dname=form.dname.data, dnumber=form.dnumber.data,mgr_ssn=result['mgr_ssn'],mgr_start=form.mgr_start.data)
        db.session.add(dept)
        db.session.commit()
...
def update_dept(dnumber):
    dept = Dept.query.get_or_404(post_id)
    ...
    if form.validate_on_submit():
        ...
        return redirect(url_for('dept', post_id=post.id))
    elif request.method == 'GET':
